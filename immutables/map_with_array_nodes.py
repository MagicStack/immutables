...
class BitmapNode:
    ...
    def assoc(self, shift, hash, key, val, mutid):
        ...
        if self.bitmap & bit:
            ...
            if key_or_null is None: # this doesn't work key may be None
            ...
            if n >= 16: # look for this
                jdx = map_mask(hash, shift)

                # error maybe here
                empty = BitmapNode(0, 0, [], mutid)
                new_array = [None] * HAMT_ARRAY_NODE_SIZE

                new_array[jdx], _ = empty.assoc(
                    shift + 5, hash, key, val, mutid)
                j = 0
                for i in range(0, HAMT_ARRAY_NODE_SIZE, 1):
                    if (self.bitmap >> i) & 1 != 0:
                        assert new_array[i] is None
                        if self.array[j] is None:
                            new_array[i] = self.array[j+1]
                        else:
                            rehash = map_hash(self.array[j])
                            new_array[i], _ = empty.assoc(
                                shift + 5, rehash,
                                self.array[j], self.array[j+1],
                                mutid)
                        j += 2
                new_node = ArrayNode(n + 1, new_array, mutid)
                if __debug__:
                    new_node._validate()
                ...
                #     return self, True
                # this fails because of the array code
                # else:
                return BitmapNode(
    ...
    def find(self, shift, hash, key):
        ...
        assert val_idx < self.size

        key_or_null = self.array[key_idx]
        val_or_node = self.array[val_idx]

        if key_or_null is None: # key maybe None
...
class ArrayNode:
    def __init__(self, count, array, mutid):
        assert count >= 0
        assert count <= HAMT_ARRAY_NODE_SIZE
        self.count = count
        assert isinstance(array, list) and len(array) == HAMT_ARRAY_NODE_SIZE
        self.array = array
        self.mutid = mutid
    ...
    def clone(self, mutid):
        if __debug__:
            self._validate()
        return ArrayNode(self.count, self.array.copy(), mutid)

    def assoc(self, shift, hash, key, val, mutid):
        idx = map_mask(hash, shift)
        node = self.array[idx]
        if node is None:
            # error is here probably
            child, added = BitmapNode(0, 0, [], mutid).assoc(
                shift+5, hash, key, val, mutid)
            if mutid and mutid == self.mutid:
                new_node = self
            else:
                new_node = ArrayNode(self.count, self.array.copy(), mutid)
            assert new_node.array[idx] is None
            new_node.array[idx] = child
            new_node.count += 1
            new_node._validate()
        else:
            child, added = node.assoc(shift+5, hash, key, val, mutid)
            if child is self: # should be child == node?
                return self, added
            if mutid and mutid == self.mutid:
                new_node = self
            else:
                new_node = self.clone(mutid)
            new_node.array[idx] = child
            new_node._validate()
    ...
    def without(self, shift, hash, key, mutid):
        ...
        if node is None:
            return W_NOT_FOUND, None
        res, sub_node = node.without(shift+5, hash, key, mutid)
        if res is W_NOT_FOUND:
            assert sub_node is None
            return res, None
        if res is W_NEWNODE:
            ...
            if mutid and mutid == self.mutid:
                ...
                target = self.clone(mutid)
            target.array[idx] = sub_node
            return res, target
        if res is W_EMPTY:
            assert sub_node is None
            ...
            if new_count == 0:
                ...
                return W_EMPTY, None
            if new_count >= 16: # should be > 16, shoudn't cause problems though
                if mutid and mutid == self.mutid:
                    target = self
                else:
                    target = self.clone(mutid)
                target.array[idx] = None
                target.count = new_count
                return W_NEWNODE, target

            bitmap_size = new_count * 2
            bitmap = 0
            ...
            for i in range(0, HAMT_ARRAY_NODE_SIZE, 1):
                ...
                if node is None:
                    continue
                bitmap |= 1 << i
                if type(node) is BitmapNode:
                    if node.size == 2 and node.array[0] is not None:
                        new_array.append(node.array[0])
                        new_array.append(node.array[1])
                    else:
            ...
            return W_NEWNODE, BitmapNode(bitmap_size, bitmap, new_array, mutid)
    ...
    def dump(self, buf, level):
        pad = '    ' * (level + 1)
        buf.append(
            pad + 'ArrayNode(count={} id={:0x}):'.format(
                self.count, id(self)))
        ...
        for i in range(0, HAMT_ARRAY_NODE_SIZE, 1):
            ...
            if node is None:
                continue
            buf.append('{}{}::'.format(pad, i))
            node.dump(buf, level + 1)
